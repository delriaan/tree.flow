  ...
  , public  = { list(
      ...
      initialize = function(init.values, init.key, name){ 
        ...
        # Initialize the root:
          ...
          self$root$`#`     <- self$root;  
          self$root$values  <- data.table::as.data.table(init.values, key = init.key);
        ...
        # Set private objects:
          ...
          private$graph.engine <- \(){
            ...
            .graph_expr <- str2lang(sprintf(
              "igraph::graph_from_literal(%s)"
              , purrr::map(.branches, pluck, "name") |> 
                  stringi::stri_replace_all_fixed(">", "+", vectorize_all = FALSE) |> 
                  paste(collapse = ", ")
              ))
              
            .graph <- eval(.graph_expr);
            
            igraph::V(.graph)$title <- purrr::map(.branches, pluck, "leaf.count") |> 
              purrr::modify_if(rlang::is_empty, ~0) |>
              unlist(use.names = FALSE) |> 
              sprintf(fmt = "Unique values: %s");
            
            igraph::V(.graph)$level <- igraph::distances(.graph, mode = "out") |> (`[`)(1,) + 1;

            cli::cli_alert_info("Edge Labels")
            .ends <- igraph::ends(.graph, igraph::E(.graph))
            # browser()
            igraph::E(.graph)$label <- purrr::map(.ends[, 1], \(i){
                x <- .branches[[i]]
                e1 <- x$prev$rule$def |> as.list() |> _[[1]] |> rlang::expr_text()
                e2 <- identical(x$prev$true, x)
                sprintf("%s [%s]", e1, e2 )
              }) |> 
              purrr::modify_if(rlang::is_empty, ~NA)
            
            cli::cli_alert_info("Edge Titles")
            igraph::E(.graph)$title <- purrr::map(.ends[, 1], \(i){
                x <- .branches[[i]]
                x$rule$desc
              }) |> 
              purrr::modify_if(rlang::is_empty, ~NA) 
      ...
      manage.rules = function(..., rem = FALSE, rtype = "list", .debug = FALSE){ 
        ...
        this.rule <- list(rlang::list2(...), rtype = rtype, rem = rem) |>
          purrr::pmap(\(...){
            if (...elt(2) == "list"){ 
              rlang::new_box(append(...elt(1), list(rem = ...elt(3)))) 
            } else if (...elt(2) == "nlist"){ 
              .val = ...elt(3); 
              purrr::map(...elt(1), \(x){ x$rem <- .val; x }) 
            } else { 
              rlang::new_box(cbind(...elt(1), rem = ...elt(3))) 
            }
          }) |> 
          purrr::flatten() |> 
          reduce(rbind);
        ...
        sub.fn <- function(rule, action){
          ...
          if (check.args[1]){ 
            ...
            if (is.character(rule$def)){ 
              rule$def <- parse(text = rule$def) 
            }
        ...
        data.table::as.data.table(this.rule) |>
          purrr::pwalk(function(...){ 
            sub.fn(
              rule = list(...)[c("def", "desc", "name")]
              , action = list(...)[[which(...names() == "rem")]]
              ) 
          });
      ...
      set.branch = function(branch = self$curnt, new.name = self$curnt$name, new.rule = self$curnt$rule){
        ...
        { if (is.character(new.rule)){ 
            self$rule.book[[new.rule]] 
          } else { new.rule }
        } |> 
        (\(x){
          if (!identical(x, branch$rule)){ 
            branch$rule <- x 
          } else { 
            message("Rules are identical: no action taken") 
          }
        })()
      
        if (and(new.name != branch$name, !(new.name %in% names(private$envs)))){ 
      ...
      del.branch = function(branch = self$curnt, confirm = TRUE){
        ...
        # private$envs <- private$envs |> (\(envs) envs[!is.na(envs)])();
      ...
      make.decision = function(branch = self$curnt, .debug = FALSE){
        ...
        # if (rlang::is_empty(branch$rule$def)) browser()
        branch <- if (is.character(branch)){ private$envs[[branch]] } else { branch }
        private$curnt.idx <- branch$id;
        ...
        if (rlang::is_empty(result)){
          cli::cli_warn("Empty rule for branch `{branch$name}`: all values will be returned ...")
          no_leaves <- TRUE
        }
        ...
        branch$true$values  <- if (no_leaves) { branch$values } else { 
            branch$values[(result) , .SD, keyby = c(self$tree.key)] 
          }
        
        branch$false$values <- if (no_leaves) { branch$values } else { 
            branch$values[(!result), .SD, keyby = c(self$tree.key)] 
          }
        
        invisible(purrr::map(c("true", "false"), \(x){ 
            branch[[x]]$leaf.count <- if (no_leaves){ 0 } else { 
              branch[[x]]$values[[self$tree.key]] |> count_leaves() 
            }
          }));
      ...
      populate = function(branch = "root", .debug = FALSE){ 
        ...
        purrr::map(
          c("true", "false")
          , \(x){
            if (!is.null(branch[[x]]$id)){
              self$populate(branch = branch[[x]]$id, .debug = .debug)
            }
          });
        
        # debug(private$graph.engine)
        invisible(private$graph.engine())
      ...
      plot = function(..., igraph.args = NULL, layout.args = list(visHierarchicalLayout = list()), refresh = FALSE){ 
        ...
        do.call(
          .layout.func
          , args = rlang::list2(graph = .viz, !!!(private$vizArgs$layout.args[[1]]))
          );
      }
    )
  }
